  ...
  render() {
    ...
    return (
      <>
          ...
          <p className="review">{[...Array(parseInt(this.props.previousReview.reviewStars,10))].map((e, i) => <span className="stars" key={i}><i className="hearts fas fa-heart"></i></span>)} to <strong><i>{ this.props.otherName.split(' ')[0]  }</i></strong> for <strong><i>{ this.state.serviceInfo && this.state.serviceInfo.name }</i></strong> </p>
      </> 
    );
  }
